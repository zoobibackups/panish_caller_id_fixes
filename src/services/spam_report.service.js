...
async function create(params) {
	...
	console.log(userId, mobile_number, country_iso2, name);
	return new Promise(async (resolve, reject) => {
						...
						.then(async (connection) => {
							return findOrCreateFilteredContact(name, mobile_number, country.id, user.id, connection)
								.then((filtered_contact) => {
									return InsertOrUpdateSpamReport(name, filtered_contact.filtered_contact_id, user.id, connection)
										.then((data) => {
											connection.release()
											resolve({
												status: true,
...
async function InsertOrUpdateSpamReport(name, fcid, uid, connection) {
	return new Promise(async (resolve, reject) => {
		let insertQuery = `INSERT IGNORE INTO spam_report (id, filtered_contact_id, user_id) VALUES (NULL, '${fcid}', '${uid}') ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id)`;
		try {
